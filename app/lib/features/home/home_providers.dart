...
final dailySummaryProvider =
    FutureProvider.family<DailySummary, String>((Ref ref, String date) {
  return ref.watch(summaryRepositoryProvider).daily(date);
});

/// 某日餐次列表(API §4.12)。
final mealsByDateProvider =
    FutureProvider.family<List<MealEntry>, String>((Ref ref, String date) {
  return ref.watch(mealRepositoryProvider).mealsByDate(date);
});

/// 近 N 天趋势(API §4.15)。
final trendProvider = FutureProvider.family<List<TrendDay>, int>((Ref ref, int days) {
