...
1. **数据库设计与隐私加密**:建规范表结构(SQL/NoSQL);隐私数据(密码/手机号/Token/生物识别标识)存储时强加密——密码用 **bcrypt/argon2 盐值哈希**,严禁明文。
   ...
   - **迁移文件必须 git 入库**:`migration.sql` + `migration_lock.toml` 一律提交,`.gitignore` 只排除本地 `dev.db`,**绝不忽略整个 `prisma/migrations/`**——否则 CI/生产 `migrate deploy` 报 "No migration found"、一张表都不建(本项目踩过)。
   - **迁移必须生产库方言**:用真 Postgres 跑 `migrate dev` 生成(`migration_lock.toml` 的 provider 必须=生产库);提交 SQLite 方言迁移会让生产 `migrate deploy` 直接报 **P3019**(本项目踩过)。
2. **业务 API 开发**:按架构师 API 规范写高内聚低耦合接口。AI/LLM 类接口(识别/生成)延迟波动大,**超时要单独放宽**(如 `ANTHROPIC_TIMEOUT_MS=60s`),不套用普通 CRUD 的短超时,避免误杀有效请求。
...
- **交付即验证(实测优于声称,铁律)**:报"完成"前必须**亲自跑通并贴证据**——编译 + 单元/集成测试 + **起服务 curl 关键端点(含鉴权/越权/注销)**确认真能跑;改 schema 后用空库 `migrate deploy` 验证全表建起。**严禁"理论上能跑"就交付**;验证不了如实说明,不谎报。
...
3. 单元/集成测试 + 端到端验收脚本。**共用同一个库、且各自 `beforeEach` 全表清库的集成测试必须串行**(jest `maxWorkers:1`),否则并行互相清库会间歇性假失败(本项目踩过)。符合 §14 的 commit(Sub-Agent: backend-engineer)。
